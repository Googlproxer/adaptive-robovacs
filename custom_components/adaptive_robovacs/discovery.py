...
class RobotProfile:
    ...
    def supports_mopping(self) -> bool:
        ...
        return bool(
            self.mop_mode_select_entity_id or self.mop_intensity_select_entity_id
        )
...
class DiscoverySnapshot:
    """One immutable registry-discovery result."""

    robots: Mapping[str, DiscoveredRobot]
    rooms: Mapping[str, DiscoveredRoom]

    @classmethod
    def empty(cls) -> DiscoverySnapshot:
        """Return an empty immutable discovery snapshot."""
...
def _labels_for(
    ...
    for label_id in tuple(result):
        if label := registry.async_get_label(label_id):
            normalized = _normalised_label(label.name)
    ...
    for entry in entries:
        ...
        evidence.append(
            AdapterEntityEvidence(
                ...
                options=tuple(str(option) for option in attributes.get("options", [])),
            )
        )
    ...
    for entry in entries:
        ...
        if domain == "sensor":
            if device_class == "battery" and not profile.battery_entity_id:
                profile = replace(profile, battery_entity_id=entry.entity_id)
                continue
            sensor_name = (
                " ".join(
                    str(value or "")
            ...
            if (
                ...
                profile = replace(profile, cleaning_time_entity_id=entry.entity_id)
                continue
        if domain != "select":
            continue
        ...
        if kind == "passes" and not profile.passes_select_entity_id:
            profile = replace(
                profile,
                passes_select_entity_id=entry.entity_id,
                passes_options=options,
            )
            continue
        if kind == "mop_intensity" and not profile.mop_intensity_select_entity_id:
            profile = replace(
                profile,
                mop_intensity_select_entity_id=entry.entity_id,
                mop_intensity_options=options,
            )
            continue
        if kind == "mode" and not profile.mode_select_entity_id:
            profile = replace(
                profile,
                mode_select_entity_id=entry.entity_id,
        ...
        if kind == "mop_mode" and not profile.mop_mode_select_entity_id:
            profile = replace(
                profile,
                mop_mode_select_entity_id=entry.entity_id,
                mop_mode_options=options,
            )
    return profile


async def async_discover(hass: HomeAssistant) -> DiscoverySnapshot:
    ...
    for entry in entities.entities.values():
        ...
        device_entries = (
            entries_by_device.get(entry.device_id, []) if entry.device_id else []
        )
        profile = _find_profile(hass, device_entries, labels)
        supports_area_clean = bool(
        ...
        platform = str(entry.platform)
        adapter_entities = _adapter_evidence(hass, device_entries)
        if operation_mode := _verified_operation_mode(adapter_entities):
            profile = replace(
                profile,
                mode_select_entity_id=operation_mode.entity_id,
                mode_options=operation_mode.options,
            )
        ...
        robots[entry.entity_id] = DiscoveredRobot(
            entity_id=entry.entity_id,
    ...
    for entry in entities.entities.values():
        ...
        sources.append(
            DiscoveredOccupancySource(
                registry_id=str(
                    getattr(entry, "id", None) or f"{entry.platform}:{entry.unique_id}"
                ),
                entity_id=entry.entity_id,
                kind=kind,
            )
    ...
    served_floors = {robot.floor_id for robot in robots.values() if robot.floor_id}
    rooms: dict[str, DiscoveredRoom] = {}
    for area in areas.areas.values():
        ...
        rooms[area.id] = DiscoveredRoom(
            area_id=area.id,
            name=area.name,
            floor_id=area.floor_id,
            labels=frozenset(area_labels),
            radar_entity_ids=tuple(sorted(radars)),
            fallback_entity_ids=tuple(sorted(fallbacks)),
            occupancy_sources=tuple(
                sorted(sources, key=lambda source: source.registry_id)
            ),
        )
    return DiscoverySnapshot(
        MappingProxyType(robots),
        MappingProxyType(rooms),
    )
